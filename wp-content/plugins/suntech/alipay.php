...
class WC_Gateway_Suntech_Alipay extends WC_Gateway_Suntech_Base{

  public function __construct() {
    $this->id = 'suntech_alipay';
  ...
  public function init_from_fields(){
    ...
    $this->form_fields = array(
                        'enabled' => array(
                                ...
                                'type'    => 'checkbox',
                                'label'   => __( '啟用紅陽Alipay支付寶', 'woocommerce' ),
                                'default' => 'yes'
                        ),
                        'title' => array(
                                'title'       => __( '顯示名稱', 'woocommerce' ),
                                'type'        => 'text',
                                'default'     => __( '紅陽Alipay支付寶', 'woocommerce' ),
                                'desc_tip'    => true,
                        ),
                        'description' => array(
                                'title'       => __( '金流描述', 'woocommerce' ),
                                'type'        => 'textarea', 'description' => __( '顯示在前台的描述文字', 'woocommerce' ),
                                'default'     => __( '進入紅陽Alipay支付寶流程', 'woocommerce' )
                        ),
...
$_note1 = self::get_note1('alipay',$order,'billing_email');
...
function add_woocommerce_suntechalipay_gateway($methods) {
  $methods[] = 'WC_Gateway_Suntech_Alipay';
  return $methods;
} 
add_filter('woocommerce_payment_gateways', 'add_woocommerce_suntechalipay_gateway');
