...
class WC_Gateway_Suntech_Pay24 extends WC_Gateway_Suntech_Base
{
    ...
    public function init_from_fields()
    {
        $this->form_fields = array(
            'enabled' => array(
                ...
                'type' => 'checkbox',
                'label' => __('啟用紅陽超商代收_付款單列印(24Payment)', 'woocommerce'),
                'default' => 'yes'
            ),
            'title' => array(
                'title' => __('顯示名稱', 'woocommerce'),
                'type' => 'text',
                'default' => __('超商代收(列印繳費單)', 'woocommerce'),
                'desc_tip' => true,
            ),
            'description' => array(
                ...
                'default' => __('超商代收(列印付款單)，自行列印條碼帳單後至超商付款', 'woocommerce')
            ),
            ...
            'order_button_text' => array('title' => '前台按鈕顯示文字', 'type' => 'text', 'default' => '取得繳費單'),
            'due_date' => array('title' => '付款期限(天)', 'type' => 'text', 'default' => '7'),
            'shipment' => array(
    ...
    function receipt_page($order_id)
    {
        ...
        $_note1 = self::get_note1('pay24', $order, 'billing_email');
...
//
        ...
        update_option($opt_name, serialize(array(
            'DueDate' => $this->due_date, 'UserNo' => $_UserNo, 'str_input_product' => $_str_inputs_product,
...
function add_woocommerce_suntechpay24_gateway($methods)
{
    $methods[] = 'WC_Gateway_Suntech_Pay24';
    return $methods;
}

add_filter('woocommerce_payment_gateways', 'add_woocommerce_suntechpay24_gateway');
