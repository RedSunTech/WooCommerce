...
class WC_Gateway_Suntech_Webatm extends WC_Gateway_Suntech_Base{

  public function __construct() {
    $this->id = 'suntech_webatm';
  ...
  public function init_from_fields(){
    ...
    $this->form_fields = array(
                        'enabled' => array(
                                ...
                                'type'    => 'checkbox',
                                'label'   => __( '啟用紅陽WebATM', 'woocommerce' ),
                                'default' => 'yes'
                        ),
                        'title' => array(
                                'title'       => __( '顯示名稱', 'woocommerce' ),
                                'type'        => 'text',
                                'default'     => __( '紅陽WebATM', 'woocommerce' ),
                                'desc_tip'    => true,
                        ),
                        'description' => array(
                                ...
                                'default'     => __( '進入紅陽WebATM流程', 'woocommerce' )
                        ),
...
$_note1 = self::get_note1('webatm',$order,'billing_email');
...
function add_woocommerce_suntechwebatm_gateway($methods) {
  $methods[] = 'WC_Gateway_Suntech_Webatm';
  return $methods;
} 
add_filter('woocommerce_payment_gateways', 'add_woocommerce_suntechwebatm_gateway');
